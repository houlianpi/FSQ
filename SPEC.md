...
The user configuration schema is version 2 and stores the active Provider metadata together with an ordered registry of workspace name and normalized absolute `.fsq/config.yaml` path entries. A valid version-1 Provider configuration is atomically upgraded by the first operation that needs the combined model, without changing Provider metadata or credential files. Provider and workspace-registry writes share one in-process lock and serialize the complete current user model so neither operation drops the other's state. Registry names are case-insensitively unique, config paths are unique under host path-case semantics, and invalid or missing registered workspaces remain visible as unavailable entries.
...
A workspace is valid only when `<workspace-root>/.fsq/config.yaml` is a readable, strictly validated version-1 workspace document whose normalized `root_path` identifies that root. It owns immutable name/platform identity, the platform-specific application target, and an `env` mapping whose keys are the effective runtime-secret allowlist and whose non-empty string values are the private in-memory secret source. Workspace settings compose the selected platform preset, repository-resolved skill resources, workspace target/paths/secrets, retained system settings, and latest Provider snapshot. Effective workspace paths are `<root>/cases` for cases and the direct run/output root, and `<root>/knowledge` for project knowledge. Loading never creates `.fsq`, a config file, or a workspace marker; a deleted `cases/` may be recreated only at a run-write boundary, and missing knowledge remains absent.

Only `FSQ_ANDROID_SERIAL`, `FSQ_WINDOWS_BACKEND_KIND`, and `FSQ_MACOS_APPIUM_SERVER_URL` remain environment-backed platform settings. Android app id, Web browser executable path, Windows app path/window matcher/launch args, and macOS bundle id/app path come only from the workspace target and are not overridden or recovered from environment variables or legacy preset fields. Workspace runtime-secret values remain private settings data, are not injected into `os.environ`, and are the common source for execution-time lookup and redaction.

`fsq-agent run`, `report`, and `playground` validate the current directory as the workspace root and derive platform from its config; they do not accept `--platform`. `fsq-agent init` remains a recognized command but performs no writes and exits nonzero with guidance to create a workspace through Control Plane because workspace initialization and migration are not implemented. Legacy `.fsq-agent-workspace` directories are incompatible with these commands and are not migrated. Runtime GitHub Copilot construction may silently exchange a valid cached GitHub OAuth token for a fresh short-lived provider token, but it must not start device-code authentication. If no provider is configured or the saved provider is not runtime-ready, provider-dependent execution fails with a setup-required configuration error that directs the user to Control Plane Config.

## Workspace Runtime Boundary

Each workspace represents one tested application on exactly one platform. Its authored cases may occur anywhere below `cases/`; every dynamic or strict run creates one unique direct child directory below `cases/`, and all events, evidence, reports, recordings, AgentTool artifacts, and generated files for that run remain inside it. Case inputs and strict lifecycle `runCase` dependencies must resolve inside the current workspace `cases/` tree. AgentTool reads are limited to workspace cases and knowledge, repository preset skill resources, and the current run as needed; writes are limited to the current run artifact boundary.

Control Plane workspace management reads the user registry and explicit registered paths and may start from any directory. Its trusted-local workspace APIs create, list, read, update, and browse workspaces while preventing identity changes and all browser file mutation. Control Plane opens on static Overview with the Workspace navigation group expanded and no selected workspace. Browser workspace selection is in-memory only. The existing Devices feature is an explicit current exception: it remains independent from selected workspace state and continues to write below `<control-plane-start-directory>/.fsq-agent-workspace/output/runs` until its own workspace integration contract changes.
...
- Public `run`, `report`, and `playground` derive the active platform from the current directory workspace config; config loading maps that immutable platform id to the corresponding repository-owned `config.<platform>.yaml` preset before validation. Provider setup belongs only to the local Control Plane Config page.
...
- Repository-owned platform YAML presets own stable platform defaults and policy. Workspace config owns application target fields and runtime-secret values. The user-level store owns Provider selection/credentials and the workspace registry. Environment variables own only the retained shared system settings documented above.
...
- Application id comes from the workspace target; optional device selection comes from `FSQ_ANDROID_SERIAL` or strict FSQ case metadata where allowed.
...
- Platform id: `windows`.
- Backend: `pywinauto`.
- App path, optional launched-window title matcher, and launch arguments come from the workspace target. `FSQ_WINDOWS_BACKEND_KIND` selects pywinauto's UI automation mode (`uia` by default, or `win32`) and is not a second FSQ Windows backend.
...
- `FSQ_MACOS_APPIUM_SERVER_URL` supplies the shared local Appium endpoint. Bundle id and app path come from the workspace target. YAML owns stable macOS defaults such as backend selection, page-source simplification depth, and action timeout seconds.
...
| Module | SPEC | Purpose |
|---|---|---|
| models | fsq_agent/models/SPEC.md | Owns shared domain models, workspace config/registry/settings boundary models, FSQ case and lifecycle hook metadata/settings models, capability metadata/registry contracts, invocation/result contracts, replay reference models, and exceptions. |
...
| control_plane | fsq_agent/control_plane/SPEC.md | Serves the local Control Plane, including workspace management/browsing, Provider configuration, discovery/readiness, run orchestration, progress streaming, cancellation, and current evidence projection. |
| cli | fsq_agent/cli/SPEC.md | Exposes workspace-resolved `run`, `report`, and `playground`, non-writing `init`, `control-plane`, capability registry bootstrap, strict replay, dynamic recording, and thin local server startup workflows. |
