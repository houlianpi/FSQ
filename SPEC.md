...
`fsq-agent run`, `report`, and `playground` require `--workspace NAME`, resolve that name case-insensitively through the user workspace registry, validate the registered workspace, and derive platform from its config. Their workspace selection is independent of the process current directory, has no current-directory fallback, and does not accept `--platform`. `run` accepts an optional Android-only `--android-serial SERIAL` value for one execution. Without that option, a direct Android run selects the sole online device and fails before execution when zero or multiple devices are online. `fsq-agent init` remains a recognized command but performs no writes and exits nonzero with guidance to create a workspace through Control Plane because workspace initialization and migration are not implemented. Legacy `.fsq-agent-workspace` directories are incompatible with these commands and are not migrated. Runtime GitHub Copilot construction may silently exchange a valid cached GitHub OAuth token for a fresh short-lived provider token, but it must not start device-code authentication. If no provider is configured or the saved provider is not runtime-ready, provider-dependent execution fails with a setup-required configuration error that directs the user to Control Plane Config.
...
- Public `run`, `report`, and `playground` resolve the required workspace name through the user registry and derive the active platform from that registered workspace config; config loading maps that immutable platform id to the corresponding repository-owned `config.<platform>.yaml` preset before validation. Provider setup belongs only to the local Control Plane Config page.
...
- Repository-owned platform YAML presets own stable platform defaults and policy. Workspace config owns application target fields and runtime-secret values. The user-level store owns Provider selection/credentials and the workspace registry. Android device identity is selected only for the active run.
