...
`fsq-agent run`, `report`, and `playground` require both `--workspace NAME` and `--platform PLATFORM`, resolve the workspace name case-insensitively through the user registry, and validate the exact configured platform. Their workspace selection is independent of the process current directory and has no current-directory fallback. `run` accepts an optional Android-only `--android-serial SERIAL` value for one execution. Without that option, a direct Android run selects the sole online device and fails before execution when zero or multiple devices are online. `fsq-agent init` remains a recognized command but performs no writes and exits nonzero with guidance to create a workspace through Control Plane because workspace initialization and migration are not implemented. Legacy `.fsq/config.yaml` and `.fsq-agent-workspace` formats are incompatible with these commands and are not migrated. Runtime GitHub Copilot construction may silently exchange a valid cached GitHub OAuth token for a fresh short-lived provider token, but it must not start device-code authentication. If no provider is configured or the saved provider is not runtime-ready, provider-dependent execution fails with a setup-required configuration error that directs the user to Control Plane Config.

## Workspace Runtime Boundary

Each workspace represents one tested application on one or more independently configured platforms. Authored cases occur below `cases/<platform>/`; every dynamic or strict run creates one unique direct child directory below `.fsq/runs/<platform>/`, and all events, evidence, reports, recordings, AgentTool artifacts, and generated files for that run remain inside it. Case inputs and strict lifecycle `runCase` dependencies must resolve inside the selected platform's cases tree. AgentTool reads are limited to the selected platform's cases and knowledge, repository preset skill resources, and the current run as needed; writes are limited to the current run artifact boundary.
...
- Public `run`, `report`, and `playground` resolve the required workspace name through the user registry and require an explicit configured platform; config loading maps that platform id to the workspace-local `.fsq/config/config.<platform>.yaml` overlay and corresponding repository-owned `config.<platform>.yaml` preset before validation. Provider setup belongs only to the local Control Plane Config page.
