...
| cli | fsq_agent/cli/SPEC.md | Exposes workspace-resolved `run`, `report`, and `playground`, non-writing `init`, `control-plane`, capability registry bootstrap, strict replay including case lifecycle hook orchestration, dynamic recording, and thin local server startup workflows. |
