...
The workspace feature loads `/workspaces` as shared application truth and distinguishes registry loading, empty, partial/unavailable, error, and retry. `Create workspace` initially shows only workspace name, a read-only parent-folder field with a `Choose folder` action, and a read-only final-path preview. It has no implicit platform draft. Each `Add platform` action appends one complete platform section vertically below the workspace identity fields, containing platform selection, platform-specific target controls, and a collapsed-by-default optional Environment disclosure.
