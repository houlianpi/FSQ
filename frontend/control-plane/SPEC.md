...
The workspace feature loads `/workspaces` as shared application truth and distinguishes registry loading, empty, partial/unavailable, error, and retry. `Create workspace` initially shows only workspace name, a read-only parent-folder field with a `Choose folder` action, and a read-only final-path preview. It has no implicit platform draft. Each `Add platform` action appends one complete platform section vertically below the workspace identity fields, containing platform selection, platform-specific target controls, and a collapsed-by-default optional Environment disclosure. A Web target presents a required browser-channel selector containing every Playwright-documented Chromium distribution channel (`chromium`, Chrome stable/Beta/Dev/Canary, and Microsoft Edge stable/Beta/Dev/Canary) together with its required executable path; existing Web targets without a stored channel present as Chrome.

`Choose folder` calls the trusted-local `POST /workspaces/pick-parent-directory` transport and never accepts manual path entry. While selection is pending, duplicate selection and workspace submission are disabled without clearing other drafts. A selected response replaces the transient parent path and updates the final-path preview; cancellation preserves the prior value and restores focus to the action; failure preserves every draft and exposes a retryable scoped error without a manual-entry fallback. Stale responses after form abandonment do not repopulate state. The existing workspace-create request continues to submit the selected `parentPath` with the independent workspace name and complete platform drafts.
...
Client validation improves feedback but server validation remains authoritative. Submit validates workspace name, selected parent folder, at least one platform section, and every platform draft in that order; failure focuses the first missing or invalid control. Pending creation locks competing controls, directory selection, and duplicate submission. Server conflicts/path errors preserve every draft. Cancel clears all target/env and parent-folder drafts and restores focus to the initiating control. Successful creation refreshes registry truth, selects the new workspace, opens its main Workspace page, and focuses the workspace heading.
...
- `src/features/workspace/WorkspaceForm.tsx`: multi-platform create plus selected-platform add/edit drafts, trusted-local parent-folder selection, final-path preview, target/env validation, secret visibility, discard handling, persistence, and independent revision-conflict actions.
...
- Parent-folder selection uses a native read-only text input and native action button with visible focus, decorative folder icon, visible and programmatic busy state, cancellation focus restoration, and a narrow layout that wraps the action without changing logical tab order or causing horizontal overflow.
...
- Workspace creation obtains its transient parent path only from the trusted-local parent-directory picker, keeps workspace name independent, and preserves the existing `name`/`parentPath`/`platforms` create transport.
