...
The workspace feature loads `/workspaces` as shared application truth and distinguishes registry loading, empty, partial/unavailable, error, and retry. `Create workspace` initially shows only workspace name, parent path, and read-only final-path preview. It has no implicit platform draft. Each `Add platform` action appends one complete platform section vertically below the workspace identity fields, containing platform selection, platform-specific target controls, and a collapsed-by-default optional Environment disclosure. A Web target presents a required browser-channel selector containing every Playwright-documented Chromium distribution channel (`chromium`, Chrome stable/Beta/Dev/Canary, and Microsoft Edge stable/Beta/Dev/Canary) together with its required executable path; existing Web targets without a stored channel present as Chrome.
