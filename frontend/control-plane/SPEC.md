...
- Platform selection containing only valid configured platforms for that workspace; selecting a workspace automatically selects the platform only when exactly one valid configured platform is available, while multiple valid platforms require an explicit user selection.
...
While idle, changing workspace or platform aborts outstanding readiness/target/case requests, clears every downstream value tied to the prior context, and starts a new request generation. When a workspace change leaves exactly one valid configured platform, Devices treats that platform as the selected platform for the new generation; when zero or multiple valid platforms remain, downstream discovery waits for explicit platform selection. Once both selections exist, readiness, targets, and cases load concurrently, and a response applies only when its generation still matches both selections. Refresh reloads the same context without creating a run.
...
- Workspace selection is mounted-app memory shared by Workspace and Devices and resets on reload. Devices platform selection remains a separate state/control, is inferred only when the selected workspace has exactly one valid configured platform, remains user-selected when multiple valid configured platforms exist, and is cleared when invalid for a changed workspace.
