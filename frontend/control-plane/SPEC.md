...
Explore timeline preserves every server event in sequence order as one flat list of event cards. Strict Replay timeline is step-oriented: timeline rows are grouped by stable strict execution `stepId` and render at most one event card per strict YAML command step. Internal strict execution events such as step start, phase start/finish, harness call start/finish, artifact captured, and other per-phase progress remain available in Logs and do not render as separate timeline event cards. Strict step cards use the latest event for that step to derive the displayed label, safe message, and status without fabricating steps that do not exist in the server event stream. Timeline cards do not show timestamps. Timeline cards show a status badge only when the displayed event includes a status; events without status do not display a fallback status badge. Selectable terminal Action cards preserve label, safe message, and server-provided status.
