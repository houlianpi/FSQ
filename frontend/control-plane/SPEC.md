...
- For Strict Replay, the loaded YAML command step list derived from the selected case source.
...
Explore timeline preserves every server event in sequence order as one flat list of event cards. Strict Replay does not render the event timeline in the left operation body; it renders the selected YAML command steps in authored command order. Each Strict Replay step row shows the one-based command index, authored action name when available, canonical action name, and step kind. Internal strict execution events such as step start, phase start/finish, harness call start/finish, artifact captured, and other per-phase progress remain available in Logs and do not render as separate left-body cards. Explore timeline cards do not show timestamps. Explore timeline cards show a status badge only when the server event includes a status; events without status do not display a fallback status badge. Selectable terminal Action cards preserve label, safe message, and server-provided status.
