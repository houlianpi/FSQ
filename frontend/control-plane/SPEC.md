...
- `src/app/ControlPlaneApp.tsx`: shared workspace registry request state, refresh, available/partial selection, create initiation, workspace-configuration subpage transition, selected-browser full-bleed outlet selection, Devices selection handoff, and shell navigation projection.
...
- `src/features/workspace/WorkspaceBrowser.tsx`: full-height two-pane browser structure, bounded on-demand tree, workspace/file breadcrumb and metadata, Markdown Preview/Code tabs, non-interactive Code presentation for other text, escaped text display, request cancellation, and stale-selection protection.
...
`ControlPlaneShell` has a router-neutral page outlet and active-page callback contract and does not import feature internals. Its explicit full-bleed outlet mode changes only outlet geometry and preserves the default mode for every other presentation; it does not inspect descendant feature markup or route names. `ControlPlaneApp` owns local `overview | workspace | devices | config` page selection, main/configuration Workspace subpage state, current in-memory workspace name, selected-browser full-bleed outlet selection, top-level navigation commands, and registry navigation composition. It defaults to `overview`; durable URLs, browser storage, and a client router are not required.
...
- Workspace browsing is read-only, requests only relative `cases/`/`knowledge/` paths, never displays `.fsq`, disables raw HTML in Markdown, escapes Code content, and keeps branch/search/add/copy/commit/write controls absent. Its full-bleed outlet mode applies only to the selected browser presentation; all other pages and Workspace presentations keep default Shell geometry.
