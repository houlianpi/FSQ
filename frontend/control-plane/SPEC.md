...
The workspace feature loads `/workspaces` as shared application truth and distinguishes registry loading, empty, partial/unavailable, error, and retry. `Create workspace` initially shows only workspace name and a read-only selected-folder field with a `Choose folder` action. It does not preview a final path because the backend selects the root from current filesystem state when creation is handled. It has no implicit platform draft. Each `Add platform` action appends one complete platform section vertically below the workspace identity fields, containing platform selection, platform-specific target controls, and a collapsed-by-default optional Environment disclosure. A Web target presents a required browser-channel selector containing every Playwright-documented Chromium distribution channel (`chromium`, Chrome stable/Beta/Dev/Canary, and Microsoft Edge stable/Beta/Dev/Canary) together with its required executable path; existing Web targets without a stored channel present as Chrome.

`Choose folder` calls the trusted-local `POST /workspaces/pick-parent-directory` transport and never accepts manual path entry. While selection is pending, duplicate selection and workspace submission are disabled without clearing other drafts. A selected response replaces the transient `selectedPath`; cancellation preserves the prior value and restores focus to the action; failure preserves every draft and exposes a retryable scoped error without a manual-entry fallback. Stale responses after form abandonment do not repopulate state. Workspace creation submits `name`, `selectedPath`, and complete platform drafts; the successful response's `rootPath` is authoritative.
...
Client validation improves feedback but server validation remains authoritative. Submit validates workspace name, selected folder, at least one platform section, and every platform draft in that order; failure focuses the first missing or invalid control. Pending creation locks competing controls, directory selection, and duplicate submission. Server conflicts/path errors preserve every draft. Cancel clears all target/env and selected-folder drafts and restores focus to the initiating control. Successful creation refreshes registry truth, selects the new workspace, opens its main Workspace page with the server-selected root, and focuses the workspace heading.

### Workspace page and file browser

With no selection, Workspace shows an explicit selection/create state. An available or partial selection loads its safe summary and renders a compact full-width identity banner, not a nested/floating card, containing only immutable workspace name, root path, configured platform names/status, and the action that opens the workspace configuration subpage. The identity block shows the workspace name above the complete root path. On desktop, the platform-label group is a separate sibling to the right with visible horizontal spacing and its bottom edge aligned with the root-path line; the compact Configure action remains at the far right with a smaller, lighter label and gear icon aligned on one vertical center. At narrow widths, platform labels move below the path and Configure may occupy its own row without overlap or page-level horizontal overflow. The banner never shows target fields, env names/values, or inline Edit.
...
Below the 76px banner, the selected-workspace read-only browser fills the remaining page-outlet width and viewport height without outlet/page padding, a centered maximum-width wrapper, a separate page heading, an outer card border, or outer corner rounding. Other Workspace states retain their normal page spacing. Above 820px, the browser is a viewport-bounded `310px minmax(0, 1fr)` grid whose tree and file content scroll independently. At 820px and below, the tree precedes the file pane in normal page flow, is independently scrollable up to 320px, and the file pane remains at least 480px high.

The elevated-surface tree has a sticky Files identity header with the prototype `◧` glyph and one compact icon-only `+` action beside `Files`. The action is named `Record new case`, preserves the current workspace, and opens Devices in Explore with no platform selected so recording continues through the existing run and `Save yaml` workflow; it does not create or mutate a file directly. The expandable tree is rooted only at `cases/` and `knowledge/`, loads directories on demand, and uses compact 31px-minimum rows. Directory rows use `›` when collapsed, `⌄` when expanded, and the link-blue `■` folder glyph; YAML files use `◇`; Markdown and other files use `▧`. The selected file has both a 3px accent edge and a soft accent background. Long names elide without widening the pane.

A loaded file has a 58px-minimum path header showing the real `workspace / directories / filename`, followed by an inset bordered file panel with a 42px-minimum presentation toolbar and scrollable body. Markdown files have Preview and Code tabs with a standard labelled tab-panel relationship; Preview uses `react-markdown` without raw HTML, and Code renders escaped source in the prototype's light document-code surface. YAML files show one non-interactive Code label and render a light, read-only line-number viewer with indentation guides and restrained key/value/string syntax roles; both CRLF and LF source render without carriage-return artifacts. JSON and other accepted UTF-8 files retain escaped plain-source presentation without YAML syntax or line-number treatment. Line count and formatted size appear in the presentation toolbar except when the loaded normalized path exactly matches `cases/<android|web|windows|macos>/<relative-path>.fsq.yaml` with a non-empty relative path and lowercase suffix. For such a case, the metadata is replaced by a `Replay Case` action that preserves the current workspace, derives the platform from the path, removes the `cases/<platform>/` prefix for the Devices case path, and opens Devices in Strict Replay without starting a run. `.fsq` is never requested or displayed. With no selected file, the content pane shows explicit selection guidance without a synthetic path header or file panel.
...
Add/Change opens an accessible provider-choice dialog for Azure GPT and GitHub Copilot GPT. Selecting Azure closes the dialog and shows `Azure GPT configuration` in the main outlet. Selecting GitHub immediately requests device authentication with no model field. Choosing the current provider is allowed as a replacement flow. The previous provider remains active until an authenticated GitHub candidate and explicitly selected model are saved.
...
While idle, changing workspace or platform aborts outstanding readiness/target/case requests, clears every downstream value tied to the prior context, and starts a new request generation. Workspace changes do not infer a platform from the available configured-platform set; downstream discovery waits until both workspace and platform selection exist. Once both selections exist, readiness, targets, and cases load concurrently, and a response applies only when its generation still matches both selections. Refresh reloads the same context without creating a run.
...
Terminal states are success, failed, inconclusive, cancelled, and error. Terminal runs do not render a separate result summary card below the timeline. A completed Explore run action row remains visible outside the bounded timeline history with `Save yaml` beside `New run`; the two actions stretch evenly across the available action-row width. Activating `Save yaml` opens an accessible confirmation dialog that asks for the saved case name without a suffix, displays `.fsq.yaml` as a fixed non-editable suffix, and previews the final `cases/<platform>/<name>.fsq.yaml` destination path before the user confirms. The default case name is the suffix-free recorded run id fallback truncated to at most the first 60 characters. The editable filename text and path preview use normal text weight rather than bold emphasis. Confirming asks Control Plane to save the generated run-local `recorded.fsq.yaml` into the selected workspace platform cases directory using the confirmed name plus the fixed `.fsq.yaml` suffix, disables duplicate saves while pending, and reports success or the backend's safe failure message through an accessible result dialog. The terminal operation panel does not render the saved filename, path, or save result text below the action row. Completed Strict Replay runs do not render `Save yaml`. New run returns to the composer, preserves the last explicit workspace/platform pair when still present, and focuses the primary mode input. Terminal transitions do not force timeline or Logs scroll position.
...
- `src/app/ControlPlaneApp.tsx`: shared workspace registry request state, refresh, available/partial selection, create initiation, workspace-configuration subpage transition, selected-browser full-bleed outlet selection, one-time Devices launch intent, Devices selection handoff, and shell navigation projection.
...
- `src/features/workspace/WorkspacePage.tsx`: selected/no-selection state, safe summary loading, compact identity banner, configuration-page navigation, immutable configuration summary, canonical persisted platform tabs, unavailable guidance, selected-platform read-only summary, Add/Edit transitions, file-browser composition, and forwarding of record/replay navigation commands.
- `src/features/workspace/WorkspaceForm.tsx`: multi-platform create plus selected-platform add/edit drafts, trusted-local selected-folder selection without a final-root preview, target/env validation, secret visibility, discard handling, persistence, and independent revision-conflict actions.
- `src/features/workspace/WorkspaceBrowser.tsx`: full-height two-pane browser structure, bounded on-demand tree, record-new-case command, eligible FSQ case-path recognition and replay command, workspace/file breadcrumb and metadata, Markdown Preview/Code tabs, non-interactive Code presentation for other text, escaped text display, request cancellation, and stale-selection protection.
...
- `src/features/devices/DevicesPage.tsx`: feature composition and optional one-time Workspace launch-intent handoff.
...
- `src/features/config/hooks/useProviderConfig.ts`: Config loading, complete Azure save, GitHub authorization/model-discovery polling, discovery retry, selected-model save, cancellation, saved-provider test, and stale-request cleanup.
...
`ControlPlaneShell` has a router-neutral page outlet and active-page callback contract and does not import feature internals. Its explicit full-bleed outlet mode changes only outlet geometry and preserves the default mode for every other presentation; it does not inspect descendant feature markup or route names. `ControlPlaneApp` owns local `overview | workspace | devices | config` page selection, main/configuration Workspace subpage state, current in-memory workspace name, selected-browser full-bleed outlet selection, one-time Workspace-to-Devices launch intent, top-level navigation commands, and registry navigation composition. It defaults to `overview`; durable URLs, browser storage, and a client router are not required.

Workspace registry/summary server state and pending commands belong to the workspace feature while the selected workspace name is shared application state. Safe summaries, platform revisions, directory entries, and file content remain feature state. Complete target/env values load only for the selected available platform after entering Edit; create/add/edit drafts are separate from safe loaded truth. The selected folder, picker request/error state, and picker request generation are transient create-form state. Directory disclosure and Preview/Code selection remain transient in the file browser. Changing workspace/subpage/platform aborts stale requests and clears private values before loading new detail.
...
Config server state, pending request state, the Azure draft, and safe GitHub transaction/model-list state are local to the Config feature. The selected pending GitHub model, dialog focus, key visibility, copy feedback, and result disclosure are transient component state. Dirty state, GitHub Save eligibility, and Test connection eligibility are derived. Config state is independent from Devices run state; changing Provider does not mutate an active run.
...
- State boundary: the application owns active page, current in-memory workspace selection, and the consumed-once Workspace-to-Devices launch intent; Overview is static; Workspace, Devices, and Config each own their server/request/interaction state; dialogs and file/timeline views own transient disclosure state.
...
Workspace registry/summary/platform-detail failures preserve the last safe truth only when identified as stale and visibly unavailable; they do not synthesize empty success. Selected-folder picker cancellation is a successful no-change outcome; picker failure preserves the prior selection and all other create drafts, remains scoped to the picker control, and offers retry without text-entry fallback. Create/add failures preserve all draft values and clear them only on explicit cancel/confirmed navigation/success. Update failures preserve loaded disk truth and the selected platform's unsaved draft separately. A revision conflict offers Reload latest and never silently resubmits. Directory/file failures remain scoped to the selected node/content pane. Frontend errors and diagnostics never include env values or unrestricted response bodies.
...
- Long workspace paths wrap or elide without resizing controls and expose the full value accessibly. The selected Workspace title preserves name-then-path reading order, bottom-aligns its desktop platform labels with the path line, and moves labels below the path when narrow; its compact Configure action retains a native button, accessible name, tooltip, keyboard activation, and visible focus state. Workspace status uses text/icons as well as color, and successful create/navigation restores focus to the workspace heading.
...
- The icon-only `Record new case` control has an accessible name and tooltip, and `Replay Case` keeps the same visible and accessible action name. Both are native keyboard-operable buttons with stable compact dimensions, visible focus, and narrow layouts that do not overlap the Files identity, presentation mode, or source content.

## Verification Scope

- A clean lock-file install, TypeScript check, focused frontend tests, and Vite build validate the entry.
- Shell/Overview tests prove one centralized sidebar renders Overview, expanded Workspace navigation, Devices, Runs, Config, and Settings in the specified order without feature imports; cover Overview default/reload reset, available/unavailable semantics, registry order, platform/status metadata without parent or root paths, selection, keyboard order, `aria-current`, narrow drawer, navigation commands, exact prototype copy and sample semantics, workflow scrolling, and focus restoration.
- Workspace tests cover malformed-response rejection; registry loading/empty/partial/unavailable/error/retry; safe summary/env omission; canonical platform ordering; all four target forms; multi-platform draft add/remove/clearing; selected/cancelled/malformed selected-folder responses; read-only selection, pending locking, replacement, retry, focus restoration, stale-response rejection, and long/narrow path layout without a final-path preview; collapsed env rows; secret visibility; whole-create validation/submission locking; create conflict/error/focus; post-create authoritative-root selection; compact banner identity order, complete path, platform labels, and Configure semantics; configuration-subpage return/tab behavior; unavailable platform guidance; additive platform save; no persisted delete affordance; edit-only private detail loading; clean/dirty/discard/pending/success/failure/independent revision-conflict update; private-value cleanup; selected-browser-only full-bleed Shell mode; accessible browser/File labels; record-new-case navigation; exact eligible FSQ path recognition and nested replay-path extraction; replay absence and metadata preservation for ineligible files; on-demand bounded tree state including platform directories; real workspace/file breadcrumb and metadata; Markdown Preview/Code safety; non-interactive Code presentation for other text; escaped text; missing/binary/invalid-UTF-8/oversized failures; `.fsq` absence; and stale-request cancellation.
...
- Workspace creation obtains its transient selected path only from the trusted-local directory picker, keeps workspace name independent, sends exactly `name`/`selectedPath`/`platforms`, and treats the successful response root as authoritative without computing or previewing it in the browser.
...
- GitHub token values never enter frontend types or state; only safe transaction status and offered model id/name values do. The complete Azure key is accepted only from the loopback Config response and is masked by default.
