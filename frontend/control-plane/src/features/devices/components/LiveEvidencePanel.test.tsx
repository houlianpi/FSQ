...
it('discloses long log messages and resumes paused log following', async () => {
  ...
  const timeLevelResizer = screen.getByRole('button', { name: 'Resize Time and Level columns' });
  const messageEventResizer = screen.getByRole('button', { name: 'Resize Message and Event columns' });
  expect(logsTable.style.getPropertyValue('--log-time-width')).toBe('82px');
  expect(logsTable.style.getPropertyValue('--log-level-width')).toBe('58px');
  expect(logsTable.style.getPropertyValue('--log-message-width')).toBe('280px');
  expect(logsTable.style.getPropertyValue('--log-event-width')).toBe('180px');
  expect(logsTable.style.getPropertyValue('--logs-table-width')).toBe('896px');
  expect(logsTable).toHaveClass('logs-table');
  fireEvent.pointerDown(timeLevelResizer, { clientX: 0 });
  fireEvent.pointerMove(window, { clientX: 10 });
  fireEvent.pointerUp(window);
  expect(logsTable.style.getPropertyValue('--log-time-width')).toBe('92px');
  expect(logsTable.style.getPropertyValue('--log-level-width')).toBe('48px');
  expect(logsTable.style.getPropertyValue('--log-message-width')).toBe('280px');
  expect(logsTable.style.getPropertyValue('--log-event-width')).toBe('180px');
  expect(logsTable.style.getPropertyValue('--logs-table-width')).toBe('896px');
  fireEvent.pointerDown(messageEventResizer, { clientX: 0 });
  fireEvent.pointerMove(window, { clientX: 120 });
  fireEvent.pointerUp(window);
  expect(logsTable.style.getPropertyValue('--log-time-width')).toBe('92px');
  expect(logsTable.style.getPropertyValue('--log-level-width')).toBe('48px');
  expect(logsTable.style.getPropertyValue('--log-message-width')).toBe('350px');
  expect(logsTable.style.getPropertyValue('--log-event-width')).toBe('110px');
  expect(logsTable.style.getPropertyValue('--logs-table-width')).toBe('896px');
