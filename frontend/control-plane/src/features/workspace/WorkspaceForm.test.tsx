...
it('clears the previous target on platform change and submits the complete masked environment', async () => {
  ...
  const pickParent = vi.spyOn(controlPlaneClient, 'pickWorkspaceParentDirectory').mockResolvedValue({ status: 'selected', selectedPath: 'C:\\projects' });
  ...
  expect(screen.queryByText('Final path')).not.toBeInTheDocument();
...
it('submits all unsaved platform drafts in one workspace creation request', async () => {
  vi.spyOn(controlPlaneClient, 'pickWorkspaceParentDirectory').mockResolvedValue({ status: 'selected', selectedPath: 'C:\\projects' });
...
it('locks picker actions while pending and preserves selection across cancel and failure', async () => {
  let resolveSelection!: (value: { status: 'selected'; selectedPath: string }) => void;
  const firstSelection = new Promise<{ status: 'selected'; selectedPath: string }>((resolve) => { resolveSelection = resolve; });
  const pickParent = vi.spyOn(controlPlaneClient, 'pickWorkspaceParentDirectory')
    ...
    .mockResolvedValueOnce({ status: 'selected', selectedPath: 'C:\\projects two' });
  const user = userEvent.setup();
  render(<WorkspaceForm mode="create" onCancel={vi.fn()} onSaved={vi.fn()} />);
  ...
  resolveSelection({ status: 'selected', selectedPath: 'C:\\projects one' });
...
it('ignores a picker response after the create form is abandoned', async () => {
  ...
  await user.click(screen.getByRole('button', { name: 'Choose folder' }));
  view.unmount();
  await act(async () => resolveSelection({ status: 'selected', selectedPath: 'C:\\late-response' }));
