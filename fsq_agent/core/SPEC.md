...
- `drivers`: private concrete automation backends reached through public Driver interfaces and factories, plus the exact metadata-only composition imports named in `drivers/SPEC.md`.
...
Core's `_default_capabilities` and compatibility `_driver_tools` use only the named metadata-composition exceptions recorded by Drivers; metadata inspection does not instantiate or connect a backend.
...
- `EvidenceRecorder`: Durable execution-fact sink, atomic checkpoint writer, and read-only evidence recovery boundary owned by `core.evidence`.
- `EvidenceJournalSink`: Public synchronous durable-acknowledgement protocol owned by `core.interfaces`; Runner and concrete capture services receive it through composition.
...
- `fsq_agent.core`: exports `CapabilityRegistry`; its implementation is private `_capabilities.py`, not a `core.registry` subpackage.
- `fsq_agent.core.runner`: exports `StepRunner` and `StepSequenceRunner`.
- `fsq_agent.core.interfaces`: exports the documented execution, evidence, observation, driver and harness protocols plus the stable composition factories. Its SPEC owns the narrow factory-composition import exception.
...
- Dependency direction: capability and runner logic depends inward on Models, Capabilities, and Core protocols. The explicitly named Interfaces factory composition boundary may select private Drivers/Harnesses factory implementations. `_default_capabilities` and compatibility `harness._driver_tools` use only the Drivers metadata-composition exception without constructing or connecting backends. The Core root retains its documented Environment compatibility exports. Core never imports Application or transport adapters. Application injects providers, artifact stores, runtime settings, and optional Harness bindings through public Core contracts; default runtime construction goes through the stable factories.
...
- settle failures: interruption or timing failure in the configured post-action delay
...
- Durable journaling, unique source/execution/capture identity, checkpoint recovery, nullable measured timing, explicit unexecuted leaves, and independent action/evidence failures follow the canonical Runner, Evidence, Interfaces, and Models contracts. The Core root introduces no alternative recorder, recovery algorithm, or result schema.
