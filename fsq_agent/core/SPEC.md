...
The canonical platform-runtime prerequisite service used by Application belongs to `environments`; Core retains only a compatibility export. Platform environment providers own host support, commands, timeouts, bounded output, readiness, supported installation/recheck, and exact-channel browser discovery.
