...
- While YAML or Strict YAML execution produces progress events that identify an authored YAML step, the static UI should highlight and center the matching Input YAML step card. Strict YAML uses deterministic step ids; dynamic raw-YAML matching is best-effort. During execution, YAML step-card and case-title interactions must not request artifacts or change the existing Preview flow.
...
- YAML tab visibility follows run mode for Input and Recorded: Goal shows Recorded plus Progress and defaults to Progress, YAML shows Input and Recorded, and Strict YAML shows Input. Generated recorded YAML loads into Recorded without automatically selecting it. The left-side Progress tab remains available across run modes, and left-side tabs use the same underline style as the right-side tabs.
...
- After execution completes, clicking an Input or Recorded YAML step card should switch Preview to that step's artifacts. Screenshots render before structured text artifacts, before/after screenshots should compare clearly, and missing artifact kinds must not create empty regions.
