...
- When a clicked step has before and after platform observation artifacts (`ui_tree`, `page_snapshot`, or `ui_snapshot`), the Preview pane may include a read-only full-content line diff that highlights added and removed lines. Paired removed and added lines should additionally highlight their changed character ranges inline.
- The step Preview screenshot section may expose an independent resize hit area directly on its bottom border, without a separate visible divider bar. Dragging upward decreases only the screenshot region height and dragging downward increases only that region height; it must not change the UI Tree region height, and increased total content height is handled by the overall Preview scrollbar.
- The screenshot height resize hit area stays on the true outer bottom border. Horizontal screenshot overflow uses a separate inner scrolling viewport so its scrollbar remains accessible immediately above the resize border.
- Screenshot border dragging changes region and image dimensions without automatically scrolling the overall Preview; pointer capture keeps the drag continuous even when the pointer leaves the border.
- The inner screenshot viewport reserves horizontal scrollbar space before resizing so the first overflow transition cannot reduce image height and cause a visible shrink-then-grow jump.
- Before resize initialization, the screenshot section occupies the full inner viewport width and must not flex-shrink to label or intrinsic content width; fixed card width is measured only from that full-width two-column layout.
- Screenshot figure cards reset the browser's default `<figure>` margins before width measurement. Measured card width and assigned Grid track width must represent the same box, preventing repeated side-margin subtraction and first-drag shrinkage.
- Before and After screenshots remain in a stable horizontal layout. When user resizing makes the comparison wider than the screenshot region, the entire comparison row overflows horizontally and is navigated with one shared scrollbar at the bottom of the screenshot region; resizing must not switch the images to vertical layout.
- A single screenshot resizes directly from the screenshot region's pointer-down height so the first drag movement remains continuous and does not jump due to image aspect-ratio fitting.
- Screenshot resizing derives region height and image-card width from their actual pointer-down dimensions. Card width grows continuously from its rendered width using the image's natural aspect ratio; it must not infer content height from the `object-fit` image element box, preventing a first-movement jump when the initial image is width-constrained.
- The fixed-width horizontal comparison layout used during screenshot resizing is initialized after Preview layout, before pointer movement, using the currently rendered card width. The first drag movement must not switch Grid modes or trigger a layout-mode jump.
- The initialized screenshot row is centered while its full content fits the region. When it becomes wider than the region, safe alignment falls back to the start so the shared horizontal scrollbar can reach the complete left and right edges.
- The default screenshot comparison fits both Before and After within the available horizontal region without requiring scrollbar interaction. Image cards start at their rendered two-column widths; horizontal overflow is introduced only after user resizing enlarges them beyond the available width.
- The UI Tree region may expose independent resize hit areas directly on its top and bottom borders, without separate visible divider bars. Dragging the top border upward increases the region and downward decreases it while preserving the visible bottom position through Preview scrolling. Dragging the bottom border downward increases the region and upward decreases it. Neither border resize changes the screenshot region height.
