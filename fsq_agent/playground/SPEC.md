...
- Verification covers playground state/session behavior, runtime info, dynamic and strict execution adapters, strict lifecycle execution, Input YAML lifecycle display/save behavior, progress delivery, cancellation, report lookup, recorded YAML display, Goal recording publication, existing-run loading, replay/video endpoints, and step-artifact preview.
