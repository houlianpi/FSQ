...
- Before/after automatic platform observations render as normalized `ui_snapshot` artifacts and may render as a read-only full-content diff with line-level added/removed highlighting and inline highlighting for paired changed lines. Explicit observation command outputs such as Android `uiTree` and Web/desktop `uiSnapshot` remain supported when they appear as step artifacts.
