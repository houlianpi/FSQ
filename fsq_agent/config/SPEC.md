...
Load, persist, merge, normalize, and validate versioned workspace configuration, the user-level active Provider plus workspace registry, repository platform presets, private workspace runtime-secret values, and the effective runtime settings used by dynamic, strict, Playground, and Control Plane entry surfaces. Config owns workspace creation/update transactions and path validation; it does not execute runs, select Android devices, or expose workspace files over HTTP.

## Dependencies

- `models`: Uses runtime settings models plus `WorkspaceConfig`, `WorkspacePlatformCreateInput`, platform workspace target models, `WorkspaceRegistryEntry`, `WorkspaceSettings`, and `ConfigurationError`.
...
- `WorkspaceConfig`, `WorkspacePlatformCreateInput`, `WorkspaceInitResult`, platform workspace target models, and `WorkspaceRegistryEntry`: Re-exported shared boundary models used by trusted entry surfaces.
...
- `create_workspace(*, selected_path: Path, name: str, platforms: Sequence[WorkspacePlatformCreateInput], user_config_root: str | Path | None = None) -> WorkspaceStatus`: Requires an unregistered normalized name and an existing selected directory. It adopts an empty selected directory as the root, or requires and creates an absent `<selected_path>/<name>` child when the selected directory contains any entry. It validates root uniqueness and a non-empty unique platform set, constructs immutable workspace configs, creates the platform-specific config/cases/knowledge/runs layout, commits all platform configs, and then appends the root-based registry entry while preserving Provider state. Ordinary failures roll back request-created content without deleting a user-owned selected directory.
...
- Boundary models: shared workspace/platform-create/target/registry/result models come from `models`; `Settings` and `UserProviderConfig` are config runtime boundaries; project exceptions come from `models`.
...
Invalid or missing configuration raises `ConfigurationError` from `models`. YAML duplicate keys, unsupported versions, schema extras, identity/root mismatches, malformed user state, invalid target paths, invalid runtime-secret entries, stale revisions, registry conflicts, containment failures, and atomic-write failures are reported with actionable safe context and no secret values. A malformed workspace or user document never degrades to unconfigured/default state. Workspace creation rejects registered names, invalid selected directories, duplicate roots, and an existing derived child path in any form without merge or overwrite. Platform addition never overwrites an existing config, and platform update never retries a stale revision. Provider-only validation remains independent from workspace target readiness. Backend packages are imported only at runtime construction, and config validation never connects to a device, browser, application, or Appium server.
...
- Workspace creation, platform addition, and platform update are separate Config operations with no automatic fallback. Config alone derives a new root from the selected directory, and no loader or mutator infers workspace name from the root basename. Entry layers may compare complete target/env mappings to select add, unchanged, or expected-revision update behavior.
...
- pywinauto package installation is operator-managed through the `windows` extra. Workspace target owns app path/window matcher/launch args, while the Windows preset owns adapter mode.
- Appium Mac2 package installation is operator-managed through the `macos` extra. Workspace target owns bundle id/app path, while the macOS preset owns the server endpoint.
