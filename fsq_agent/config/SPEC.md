...
- Workspace-aware CLI commands resolve a required case-insensitive registry name through `load_registered_workspace`, then validate and compose settings from that registered workspace root without initializing anything. Their current directory is not a workspace selection source. The legacy marker path remains available only through `load_platform_settings` for Control Plane Devices.
