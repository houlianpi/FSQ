...
- `initialize_workspace_root(*, root_path: Path, config: WorkspaceConfig, update_existing: bool = False, user_config_root: str | Path | None = None) -> WorkspaceInitResult`: Initializes exactly one platform at an existing explicit workspace root whose configured root must match `root_path`. For an unregistered root, it may adopt a non-empty project directory while preserving unrelated content, but rejects symlink roots, non-directories, existing `.fsq` content, legacy markers, or a registry name/root conflict before writing. It creates and registers the canonical platform layout transactionally. For a registered exact root it shares the add/unchanged/controlled-update behavior and revision guarantees of `initialize_workspace`. The result contains no target or private env data.
