...
- `inspect_registered_workspace(name: str, user_config_root: str | Path | None = None, *, validate_target_paths: bool = True) -> WorkspaceStatus`: Resolves one case-insensitive registry name and independently inspects exact supported platform files, returning canonical safe availability records without target or env values. Default inspection validates target paths. Doctor uses `validate_target_paths=False` to diagnose missing targets separately; this skips only target filesystem availability checks, preserving YAML, identity, platform, registry, and root-containment validation.
- `load_registered_workspace(name: str, platform: str, user_config_root: str | Path | None = None, *, allow_unavailable_target: bool = False) -> WorkspaceConfig`: Resolves one case-insensitive registry name plus explicit platform, validates filename/platform/target and registry identity, validates target paths by default, and returns that platform's current workspace config truth. The trusted diagnostic/edit option applies only to macOS.
- Registered macOS configuration supports an explicit trusted diagnostic/edit load option that defers old target-path availability checks while retaining schema, registry identity, platform, root, and containment validation. Ordinary loads retain strict path validation. A macOS update can repair a missing old application path using the current document revision; the complete replacement target is validated before persistence. Malformed or identity-mismatched documents remain non-editable, and other platforms retain their existing load/update behavior.
