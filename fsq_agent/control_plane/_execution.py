...
async def _run_explore(prepared: PreparedRun, state: ControlPlaneState) -> None:
    ...
    try:
        ...
        result = await FsqAgent.from_settings(settings).run(task, event_sink=projection.project_run_event)
        ...
        except Exception as exc:  # noqa: BLE001 - recording never changes the run result.
            state.add_event(
                request_id,
                {
                    "time": None,
