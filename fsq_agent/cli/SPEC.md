...
- `fsq-agent init --platform android|web|windows|macos`: Initialize or verify the current directory `.fsq-agent-workspace` for one committed platform preset, validate static configuration, and print readiness for the default LLM run path, strict-core run path, and provider-backed AI assertion readiness. Static configuration or workspace failures exit nonzero. Mode-specific dependency gaps are reported as readiness failures so strict-only users are not forced to configure a Provider unless they execute cases containing `assertWithAI`. The command does not accept provider fields, write `~/.fsq`, mutate Provider-related `.env` values, or start authentication.
...
Public CLI commands do not expose `--config` or `--workspace`; `init` does not expose `--provider`. Invocations using those options fail as unknown options before Provider files or authentication state can change.
...
- Public CLI commands use the current directory `.fsq-agent-workspace` and do not expose workspace selection. Provider configuration and interactive authentication belong to Control Plane Config.
...
- Package-private `fsq_agent._strict_case_recording`: Shared post-run recorder used by CLI, Playground, and Control Plane to convert dynamic run capability events into run-local `recorded.codex.yaml` and `recording.json` artifacts.
...
- `control-plane` command handler in `_main.py`: Thin adapter that selects the current-directory `.fsq-agent-workspace`, maps host/port/browser flags into `ControlPlaneServerOptions`, and calls `run_control_plane` without loading a fixed platform or reimplementing Control Plane behavior.
...
Provider readiness failures during `init` are reported as mode-specific readiness diagnostics without modifying Provider files or starting authentication. An unsupported `--provider` option fails argument parsing before side effects.
...
- `init --platform <platform>` is only a platform-preset workspace/readiness check. CLI has no Provider setup or authentication workflow and never writes Provider-related `.env` values or `~/.fsq` files. Public commands intentionally do not accept `--workspace` or `--env-file`; they use the current working directory `.env` and `.fsq-agent-workspace` for non-provider runtime setup.
