...
Provide the public command line surface for fsq-agent: resolve and validate a required registered workspace name independently of the process current directory, derive its immutable platform, bootstrap a lightweight active-platform capability registry, run either dynamic LLM goal/reference execution or strict-core YAML execution with optional case lifecycle hook orchestration and explicit provider-backed `assertWithAI`, optionally record dynamic LLM runs into strict-replay FSQ YAML artifacts from capability replay metadata, print workspace-local reports, start the workspace-local browser Playground, and start the directory-independent local Control Plane.
...
- `config`: Resolves registered workspace identity, loads the selected workspace overlay and preset, then validates provider-only, LLM runtime, or strict-core readiness without creating workspace files.
...
- `control_plane`: Starts the local Control Plane server from CLI host/port/browser options and the startup directory retained only for legacy Devices compatibility. Workspace management comes from the user registry and does not receive a CLI-selected workspace or platform.
...
- `fsq-agent init [--platform android|web|windows|macos]`: Retain the recognized legacy invocation shape, perform no filesystem or configuration writes, explain that workspace-aware initialization/migration is not implemented, direct users to create new workspaces through Control Plane, and always exit nonzero.
- `fsq-agent run --workspace NAME --goal TEXT [--android-serial SERIAL] --tracing/--no-tracing --stream/--no-stream --stream-format concise|jsonl --record --record-on-failure`: Resolve and validate the required registered workspace name, derive its immutable platform, and run one dynamic LLM task from a natural-language goal. CLI task construction sets planning reference kind `goal` and normalized goal text; pre-planning derives ordered key actions and `verification_goal` before external UI actions. Streaming defaults to concise. A tracing override applies after settings load and before runtime validation. `--record` writes a run-local strict-replay artifact after a successful run; `--record-on-failure` additionally permits draft recording for failed or inconclusive runs and requires `--record`.
- `fsq-agent run --workspace NAME --case-yaml PATH [--android-serial SERIAL] --tracing/--no-tracing --stream/--no-stream --stream-format concise|jsonl --record --record-on-failure`: Resolve the exact lowercase `.fsq.yaml` file inside the selected workspace `cases/`, read complete UTF-8 text, and run one dynamic LLM task using a `raw_case` planning-reference envelope containing the contained source path and content. The CLI does not parse this YAML for execution or derive verifier requirements. Pre-plan derives ordered key actions and `verification_goal`; optional recording uses only persisted run events.
- `fsq-agent run --workspace NAME --case-dir PATH [--android-serial SERIAL] --tracing/--no-tracing --stream/--no-stream --stream-format concise|jsonl --record --record-on-failure`: Resolve a contained directory inside the selected workspace `cases/`, recursively discover exact `*.fsq.yaml` files, sort them, and run one independent dynamic `raw_case` task per file serially. Execution and per-run recording attempts continue after failures and produce an operational summary.
- `fsq-agent run --workspace NAME --strict --case-yaml PATH [--android-serial SERIAL] --tracing/--no-tracing`: Resolve one contained `.fsq.yaml` case, load the selected workspace settings and immutable platform, construct the active harness/driver/tool provider without external connection during bootstrap, validate the active registry, load case/config lifecycle hooks, and preflight workspace runtime-secret references without serializing private values. Strict phases remain config `onCaseStart`, case `onCaseStart`, main commands when before hooks pass, case `onCaseComplete`, and config `onCaseComplete`. Hook `runCase` dependencies resolve under the same workspace `cases/` containment boundary, including traversal and symlink checks; `runShell` remains an operator-authored local command. Canonical commands execute through `StepSequenceRunner`/`StepRunner` with centralized delay and evidence policy. Each run writes evidence and strict reports inside its unique direct workspace `cases/<run-id>/` directory and snapshots the source case there for provenance without mutating authored content. Recovery, testcase mutation, and strict recording are disabled. An explicitly authored `assertWithAI` may inject a provider-backed evaluator after provider validation.
- `fsq-agent run --workspace NAME --strict --case-dir PATH [--android-serial SERIAL] --tracing/--no-tracing`: Resolve a contained directory under the selected workspace `cases/` and execute discovered cases serially through the same strict lifecycle. Hook `runCase` files are contained dependencies, not separate top-level results. Execution continues after failed top-level cases and exits nonzero when any case or dependency fails.
- `fsq-agent report --workspace NAME --run-id ID --format markdown|json`: Resolve one LLM `report.md/json` or strict `core-report.md/json` from a unique direct child of the selected workspace `cases/`; fail when the registered workspace is unavailable or invalid, no report matches, or the run id is ambiguous.
- `fsq-agent playground --workspace NAME --host HOST --port PORT --open-browser/--no-open-browser`: Load the same selected-workspace settings and derived platform as `run`, then start the local Playground. The command blocks, serves packaged Vite assets, optionally opens a browser, and delegates behavior to `playground`. Configuration, asset, and bind failures exit nonzero; installed wheels require no Node.js runtime.
- `fsq-agent control-plane --host HOST --port PORT --open-browser/--no-open-browser`: Start the local single-user Control Plane from any current directory without selecting a new workspace or platform. The startup directory is passed only for the isolated legacy Devices output boundary. Workspace management reads the user registry and explicit workspace config paths. The command blocks, serves the Vite-generated Control Plane entry, and optionally opens the browser. The default host is `127.0.0.1` and port is `8879`; `--platform`, `--config`, and `--workspace` are unsupported.

Public CLI commands do not expose `--config`; `init` does not expose `--provider`. `run`, `report`, and `playground` require `--workspace NAME`, do not expose `--platform`, and fail argument parsing before settings load or writes when the required workspace option is missing. `init` may retain `--platform` only as a recognized compatibility argument and still always fails without writes. `control-plane` does not accept `--workspace` because it manages the registry rather than operating within one selected workspace.

Every `run` form accepts optional `--android-serial SERIAL`. For an Android workspace, CLI verifies an explicit serial is online or, when omitted, discovers exactly one online device and applies it only to the run-specific settings copy. Zero devices, multiple devices without an explicit serial, an offline/unknown explicit serial, or use with a non-Android workspace fail before agent, case, hook, harness, or driver execution. Directory runs freeze one selected Android serial for the complete command.

`--goal`, `--case-yaml`, and `--case-dir` are mutually exclusive. `--strict --goal` is invalid because strict-core execution requires authored YAML steps. `--strict --record` and `--strict --record-on-failure` are invalid because recording is a dynamic-run post-processing workflow. `--record-on-failure` without `--record` is invalid. Every case-oriented `--case-yaml` input requires the exact lowercase `.fsq.yaml` suffix. Absolute and relative case inputs resolve against workspace `cases/` and are accepted only when the resolved target remains contained; there is no current-working-directory fallback outside that root. Strict hook `runCase` paths from config-level or case-level hooks use the same suffix, containment, traversal, and symlink policy.
...
- `run` and Playground startup derive `settings.harness.platform` from the valid selected workspace and use it for readiness validation, registry bootstrap, strict harness/platform tool construction, and platform-specific error messages. `init` never loads runtime settings.
- `run`, `report`, and `playground` require a workspace registry name, resolve it case-insensitively, validate the registered config identity and root, and load settings from that root. They do not inspect `Path.cwd()` for workspace identity and have no implicit current-directory fallback. Provider configuration and interactive authentication belong to Control Plane Config.
...
- `control-plane` command handler in `_main.py`: Thin adapter that captures the startup directory only for legacy Devices compatibility, maps host/port/browser flags into `ControlPlaneServerOptions`, and calls `run_control_plane` without loading a new workspace/platform or reimplementing Control Plane behavior.
...
Input validation failures, including unsupported commands/options, invalid Control Plane ports, missing workspace names, unregistered or unavailable workspaces, invalid registered workspace identity/configuration, missing or multiple run inputs, escaped/traversing/symlinked case inputs or hook dependencies, `--strict --goal`, invalid record flags, unreadable dynamic case files, invalid strict YAML, malformed lifecycle metadata, empty case directories, missing hook cases, recursive hook chains, invalid active workspace targets, invalid Web navigation/base URL policy, invalid preset-owned platform settings, invalid Android serial selection, missing workspace secret names/private values, missing provider readiness for authored strict `assertWithAI`, unresolved reports, or missing generated frontend assets fail before external UI actions begin for the affected command, case, hook, or server. Dynamic `--case-yaml` input does not fail merely because content is invalid YAML, because that path treats it as raw planning reference text.
...
`init` always reports unavailable workspace-aware initialization/migration and exits nonzero without loading readiness, modifying Provider/workspace files, or starting authentication. An unsupported `--provider` option fails argument parsing before side effects.
...
- Verification covers CLI argument validation including rejected `--provider`, Provider-side-effect-free initialization, dynamic goal/raw-case task construction, strict-case execution entry behavior, strict lifecycle ordering/failure semantics, dynamic recording handoff, report lookup, live event formatting, and thin Playground/Control Plane server startup delegation.
- Boundary verification ensures `run`/`report`/`playground` require and resolve a valid registered workspace name independently of the current directory; Android direct runs reject invalid, missing, or ambiguous device selection before execution; all case and lifecycle paths remain under the workspace `cases/`; all run artifacts remain under that workspace; `init` never writes; Control Plane can start elsewhere; unsupported commands/options fail before side effects; secret values are never rendered; and strict/dynamic execution delegates behavior to owning modules.
...
- `init` is a recognized but unavailable compatibility command. It always exits nonzero, performs no writes, and directs workspace creation to Control Plane; retaining an optional legacy `--platform` argument does not select or initialize anything.
- `run`, `report`, and `playground` require `--workspace NAME` and intentionally do not accept `--config`, `--env-file`, or `--platform`. They resolve only registered workspace identity, validate its `.fsq/config.yaml` without initialization, derive platform from that workspace, and never use `Path.cwd()` as a workspace fallback.
- A missing registry name, unknown name, unavailable registered config, or invalid workspace fails with create-or-repair guidance and no workspace or run writes.
...
- `control-plane` is a directory-independent local product entry point for new workspace management. CLI owns only argument parsing, legacy Devices startup-directory capture, and server startup; `control_plane` owns registry-backed workspace APIs plus its retained Devices runtime. The command has no `--platform` or selected-workspace option.
