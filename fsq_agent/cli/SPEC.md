...
Public CLI commands do not expose `--config`; `init` does not expose `--provider`. `run`, `report`, and `playground` require both `--workspace NAME` and `--platform PLATFORM` and fail argument parsing before settings load or writes when either option is missing. `init` may retain `--platform` only as a recognized compatibility argument and still always fails without writes. `control-plane` accepts neither selection option because browser state manages the registry and execution context.
...
- `run` and Playground startup validate that `settings.harness.platform` matches the explicit configured platform and use it for readiness validation, registry bootstrap, strict harness/platform tool construction, and platform-specific error messages. `init` never loads runtime settings.
