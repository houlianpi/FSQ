...
- `fsq-agent run --platform android|web|windows|macos --strict --case-dir PATH --tracing/--no-tracing`: Run discovered `*.codex.yaml` files serially through the same deterministic strict-core path. Execution continues after failed cases, writes a directory-run summary, and exits nonzero when any case fails.
