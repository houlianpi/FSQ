...
- `runs` is the exact-current-Workspace history and evidence-export surface. It aggregates recorded platforms by default, supports an optional platform, and never deletes, retries, replays, cancels, follows, or changes the authoritative outcome of a Run. Export writes derived files only at Application-validated destinations.
...
For Workspace-scoped commands except creation of an unregistered Workspace, the exact current directory is the CLI workspace root. User-level Provider commands and static `case format` do not require a Workspace. Execution/setup commands require a Workspace registered at that exact normalized root with at least one valid `.fsq/config/config.<platform>.yaml`; Run history/export requires only the trustworthy readable registered root and recorded platform mapping. `.fsq-agent-workspace` markers are neither created nor accepted. Commands do not search parent directories, auto-initialize, or accept an alternate workspace flag. Platform execution additionally requires the selected platform config. Workspace-precondition failure uses `workspace.not_initialized` with guidance to use the intended registered root or initialize a Workspace. Control Plane retains explicit registered Workspace selection independent of its startup directory.
...
- Global `--output human|json|jsonl` selects presentation and defaults to `human`; it precedes the command. The `runs export` command's separate `--output PATH` selects an export file and does not change the global presentation mode.
...
All Run commands require the exact current registered Workspace root and a trustworthy platform/Run mapping; they do not require a ready Provider, Driver, application, browser, or device. Omitted platform queries recorded platforms in that Workspace; a supplied platform must be identifiable there. `list` accepts repeatable status OR filters, one mode, exact normalized Case ID, a positive integer duration suffixed by `m`, `h`, or `d`, and limit `1..200` defaulting to `20`. Filtering precedes deterministic newest-first sorting and limiting. Empty results succeed. Human columns are Run ID, platform, mode, status, start, duration, and Case/Goal; machine output contains equivalent filters, counts, truncation, entries, and safe warnings.
...
`export` delegates to Application's `export_run_report` and supports Human, JSON, JSONL, and `--non-interactive` without a browser. Exactly one required format selects the full `fsq.report/v1` JSON, JUnit, standalone HTML, or evidence ZIP bundle. Absent `--output`, Application creates `exports/<export-id>/` inside the selected Run with the format's standard filename. Explicit `--output PATH` names one destination file; relative paths resolve from the Workspace root. Existing destinations and authoritative Run/source paths are rejected, and no overwrite option is exposed. The CLI does not discover files or render reports itself.

`--baseline` selects one comparable Run and repeatable `--related-run` selects lineage-linked Runs in the same Workspace/platform. Application and Report validate source identities, stable step mappings, and hashes; invalid requests fail explicitly instead of guessing relationships. `--share-profile` selects a bounded local profile for artifact-ID selection, literal text replacement, field removal, and screenshot masks on export copies. It does not publish, upload, discover secrets, change the source Run, or claim complete anonymization.

Profile selections qualify artifacts by Run ID and cannot change machine identities, outcomes, gate, counts, measurements, or reference structure. Optional Case review declarations are digest-bound user statements for the export, not Run approval. Resource limits and omission/error semantics are shared with Report.

Human export output identifies the original Run result, report gate, format, destination, and warnings. Machine output is one operation result with Run/platform/export IDs, format, output path/file inventory, and warnings; JSONL emits one terminal result. The public report is in the requested file, not substituted for the CLI operation envelope. JUnit contains one testcase for the primary root Case/Goal invocation, with step and related-Run details as properties/anchors. Failure/error/skipped mapping and evidence completeness come from the shared Report contract.

A successful export exits `0` even when the original Run failed; its JSON/JUnit/HTML retains the non-passing conclusion. Export validation, unavailable facts, and generation failures use their own normal CLI error categories and never rewrite the original test result. CI executes Case testing once, preserves its exit status, and runs evidence export/upload even after test failure; a later export error is reported separately. Preflight failures before Run allocation expose no invented Run ID or synthetic successful report.
...
Run queries sanitize secrets and unsafe backend details and return no unnecessary absolute host paths; an explicitly supplied export destination may be returned to its caller. Successfully reading a failed historical Run exits `0`. `run.not_found` and invalid export arguments exit `2`; Workspace, platform inventory, ID conflict, invalid/unavailable facts, incompatible baseline/lineage, unsafe/existing destination, or unavailable artifact errors exit `3`; report/export generation/opening, ID allocation, and internal failures exit `5`; interruption exits `130`.
...
Run verification covers historical/current schema projection, truthful active/interrupted/unknown ownership presentation, complete normalized logs, real versus unavailable metrics, cross-format conclusions/counts, all export formats in non-interactive machine mode, destination containment/no-overwrite, baseline/lineage/profile validation, source immutability, and independent test/export exit results.
