...
Lifecycle orchestration wraps this deterministic command execution path for strict public runs. The lifecycle layer loads config-level and case-level hook metadata, resolves hook `runCase` paths, detects recursive hook chains across both hook origins, executes `runShell` commands, annotates hook case steps with lifecycle phase and hook origin metadata, and then delegates canonical FSQ command steps to the same `StepRunner`/`StepSequenceRunner` path. The deterministic command execution helper must not parse or execute lifecycle hooks by itself.
...
- Strict lifecycle tests should cover cases with only `onCaseStart`, only `onCaseComplete`, both lifecycle phases, no lifecycle phases, config-level hooks, hook entries containing only `runCase`, only `runShell`, or both in authored YAML order, config before hooks running before case before hooks, config after hooks running after case after hooks, config before failure skipping case before and main commands while still running after hooks, case before failure still running case/config after hooks, config after failure failing the overall case, recursive hook chain rejection across config and case hooks, hook `runCase` relative path resolution, shell hook exit-code handling, and strict `--case-dir` summaries attributing config/case hook failures to the owning top-level case.
