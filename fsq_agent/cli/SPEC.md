...
- `config`: Loads the current-directory workspace overlay and selected preset, then validates provider-only, LLM runtime, or strict-core readiness without creating workspace files.
...
Public CLI commands do not expose `--config` or `--workspace`; `init` does not expose `--provider`. `run`, `report`, and `playground` do not expose `--platform`, and supplying it fails during argument parsing before settings load or writes. `init` may retain `--platform` only as a recognized compatibility argument and still always fails without writes.
...
- `run` and Playground startup derive `settings.harness.platform` from the valid current workspace and use it for readiness validation, registry bootstrap, strict harness/platform tool construction, and platform-specific error messages. `init` never loads runtime settings.
- `run`, `report`, and `playground` treat `Path.cwd()` as the workspace root and require its `.fsq/config.yaml`; they do not expose workspace selection. Provider configuration and interactive authentication belong to Control Plane Config.
...
- Android strict runs require Android app id from the active workspace target according to strict validation rules; retained serial selection may come from system/platform environment settings.
...
- Windows strict runs validate `harness.windows.backend == "pywinauto"`, the workspace-owned app path/window-title/launch arguments, and the retained environment-backed pywinauto adapter setting before external UI actions begin.
...
- macOS strict runs validate `harness.macos.backend == "appium_mac2"`, the retained environment-backed Appium server setting, and the workspace-owned bundle-id/app-path target before external UI actions begin.
...
Input validation failures, including unsupported commands/options, invalid Control Plane ports, invalid or legacy current-directory workspaces, missing or multiple run inputs, escaped/traversing/symlinked case inputs or hook dependencies, `--strict --goal`, invalid record flags, unreadable dynamic case files, invalid strict YAML, malformed lifecycle metadata, empty case directories, missing hook cases, recursive hook chains, invalid active workspace targets, invalid Web navigation/base URL policy, invalid retained platform settings, missing workspace secret names/private values, missing provider readiness for authored strict `assertWithAI`, unresolved reports, or missing generated frontend assets fail before external UI actions begin for the affected command, case, hook, or server. Dynamic `--case-yaml` input does not fail merely because content is invalid YAML, because that path treats it as raw planning reference text.
...
- Boundary verification ensures `run`/`report`/`playground` require a valid current-directory workspace, all case and lifecycle paths remain under its `cases/`, `init` never writes, Control Plane can start elsewhere, unsupported commands/options fail before side effects, secret values are never rendered, and strict/dynamic execution delegates behavior to owning modules.
...
- `run`, `report`, and `playground` intentionally do not accept `--workspace`, `--config`, `--env-file`, or `--platform`. They use `Path.cwd()` only as the candidate workspace root, validate `.fsq/config.yaml` without initialization, and derive platform from that workspace.
- A current directory containing legacy `.fsq-agent-workspace` but no valid new config fails with a specific incompatible-format message; other invalid directories receive create-in-Control-Plane guidance. Neither path creates files.
...
- Android app id is workspace target truth. Android serial remains a retained system/platform environment setting; CLI exposes neither as a flag.
