...
- Verification covers CLI argument validation, provider setup and `.env` upsert behavior, dynamic goal/raw-case task construction, strict-case execution entry behavior, strict lifecycle ordering/failure semantics, dynamic recording handoff, report lookup, and live event formatting.
...
- The public command surface is limited to `init`, `run`, `report`, and `playground`. Unsupported command names are not retained as compatibility aliases.
