...
- Sensitive runtime-secret text input values must be redacted in reports. Reports may show safe metadata such as requested environment variable name, text source type, allowlist/presence status, capability name, and replay alias, but never raw secret values. Historical `get_runtime_secret` dependency events are not part of the target runtime-secret input path and need not be treated as active recording dependencies.
