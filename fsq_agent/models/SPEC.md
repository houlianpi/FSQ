...
- `WebHarnessSettings`: Pydantic model for Web runtime construction. The preset owns Playwright backend/channel/headless/base-URL/viewport policy and the workspace target supplies the required browser executable path. Strict-core execution does not enable AI assertion evaluators through this settings model.
...
- `AndroidWorkspaceTarget`, `WebWorkspaceTarget`, `WindowsWorkspaceTarget`, and `MacOSWorkspaceTarget`: Strict Pydantic target variants selected by workspace platform. They own application target values only and reject unknown fields.
