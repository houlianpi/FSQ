...
- `RuntimeSecretSettings`: Pydantic model containing the runtime-secret names available to text-entry commands and their private non-serialized values. Workspace `env` keys define the allowlist and values; concrete values remain in memory and never enter YAML cases, model-facing settings serialization, events, reports, or recordings.
...
- `WorkspaceConfig`: Strict extra-forbid version-1 Pydantic boundary model for immutable workspace name/root/platform identity, its matching target variant, and runtime-secret `env`. Names are trimmed bounded single-directory names and reject empty, dot segments, separators, control characters, host-invalid forms, and applicable Windows reserved device names. Env keys are valid environment-variable names and values are non-empty strings; duplicate YAML keys are rejected by config before model validation.
- `WorkspaceRegistryEntry`: Pydantic model containing one immutable registered workspace name and normalized absolute `.fsq/config.yaml` path without target or secret data.
- `WorkspaceSettings`: Pydantic model for the validated workspace root and config path. It has no marker/autoinit setting and does not imply filesystem creation.
- `CaseSettings`: Pydantic model for the read-only FSQ case directory.
- `OutputSettings`: Pydantic model for the managed run/output root. Workspace composition sets both output roots to workspace `cases/`; every run creates one unique direct child and all logs, reports, tool artifacts, and generated files remain inside that run directory. There is no separate workspace `output/` root.
...
- `_settings.py`: Settings value models, versioned workspace config/target models, registry entry model, and private runtime-secret value contract.
...
- Workspace config is a strict versioned boundary. `platform` and `target` must select matching discriminated target models; `root_path` must resolve to the containing workspace root; names and `env` keys/values obey config-owned validation; and shared models never perform filesystem IO.
