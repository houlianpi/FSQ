...
- `WorkspaceConfig`: Strict extra-forbid version-2 Pydantic boundary model for one platform's immutable workspace name/root/platform identity, matching target variant, and private runtime-secret `env`. Names are trimmed bounded single-directory names and reject empty, dot segments, separators, control characters, host-invalid forms, and applicable Windows reserved device names. Env keys are valid environment-variable names and values are non-empty strings; duplicate YAML keys are rejected by config before model validation.
