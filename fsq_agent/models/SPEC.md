...
- `AndroidHarnessSettings`: Pydantic model for Android runtime construction. The preset selects backend policy, the workspace target supplies `app_id`, and `FSQ_ANDROID_SERIAL` may select a device. Strict-core execution does not enable AI assertion evaluators through this settings model.
...
- `WindowsHarnessSettings`: Pydantic model for Windows runtime construction. The preset selects `pywinauto`; the workspace target supplies application path, optional window title regex, and launch arguments; `FSQ_WINDOWS_BACKEND_KIND` selects the pywinauto adapter mode. Strict-core execution does not enable AI assertion evaluators through this settings model.
- `MacOSHarnessSettings`: Pydantic model for macOS runtime construction. The preset selects `appium_mac2` and stable timeouts/snapshot policy, the workspace target supplies bundle id and/or app path, and `FSQ_MACOS_APPIUM_SERVER_URL` supplies the shared local server endpoint. Strict-core execution does not enable AI assertion evaluators through this settings model.
...
- Windows launch defaults combine workspace-owned app path/window title/launch arguments with the retained environment-backed pywinauto backend kind. `WindowsLaunchAppParams.extra_args` remains a per-step append-only override rather than a replacement for settings ownership.
