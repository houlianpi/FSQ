...
- `ToolCallRecord`: Pydantic model for a normalized real tool invocation reconstructed from run events, including true tool name, origin (`agent_tool`, `common`, `platform`, `runtime`, or `unknown`), safe arguments/output, evidence references, timing, and errors. Transport completion and actual execution status are distinct fields; a completed SDK call may contain a failed capability result. The compatibility `status` field retains its transport meaning and is not the action verdict.
...
- `TaskResult`: Complete-operation compatibility result returned by the Execution-backed coordinator after execution, verification, and report processing. Its required `ReportArtifact` can reference the frozen minimal execution result when rich reporting fails.
...
- `RunMetadata`, `RunSource`, `RunResultSummary`, `RunStepCounts`, `RunRuntime`, and `RunArtifactIndex`: canonical immutable Run value models. Execution and Application convenience exports reference these same classes rather than defining duplicates. `RunMetadata` writes `fsq.run/v2` and reads supported `fsq.run/v1` values without inventing unavailable fields.
- `RunExecutionContext`: immutable SDK-neutral allocated Run identity, platform, Run directory, and safe provenance values used by Execution and Agent. It contains no live provider, harness, event sink, coordinator, or cancellation callback; the trusted local directory is not serialized into public reports.
- `DynamicAgentOutcome`: SDK-neutral dynamic execution and verification return value without a required report artifact. It preserves task identity, final task/verification goal, runtime results, actual invocation references, safe errors, and measured timing for Execution-owned finalization.
- `RunExecutionResult`: immutable `fsq.execution-result/v1` facts for completed or partial execution, verification, logical-step counts, primary failure, and required evidence health. Report and recording outcomes do not change it.
- `EvidenceJournalRecord`: `fsq.evidence-event/v1` envelope for one monotonic sequence, Run identity, timestamp, source/execution identity, event kind, and typed safe execution fact. It is distinct from Agent `RunEvent` and does not contain transport projections.
- `PublicRunReport`: versioned `fsq.report/v1` contract for Run identity, results, source/provenance, logical steps and attempts, safe logs, artifacts, scoped metrics, processing results, lineage, comparison, and warnings. It contains persisted or explicitly derived facts, not live runtime objects.
- `RunReportExportOptions` and `RunReportExportResult`: Report boundary values for one format (`json`, `junit`, `html`, or `bundle`), an Application-validated destination, optional validated share transformations, and generated file inventory. Local paths in these trusted internal boundary values are not browser response fields.
- `RunShareProfile`: Public validated share-profile boundary containing artifact selections, literal replacements, field removals, screenshot masks, and optional digest-bound review declaration. Its nested record classes are implementation-owned schema components, not additional package exports.
...
- `AndroidInputTextParams`: Pydantic model for `input_text` parameters. It requires string `text`, optional serialized `textType` defaulting to `literal`, and either a `target` or non-empty `locator`. When `textType="runtimeSecret"`, `text` names a Workspace runtime-secret reference resolved by `core` immediately before driver invocation; concrete Android drivers receive only resolved literal strings.
...
- `WebLocator`: Pydantic model for Web target locators with optional `role`, `name`, `text`, `label`, `placeholder`, `testId`, `css`, `xpath`, `altText`, `title`, and `ref` fields. Web action parameter models that accept either a semantic `target` or `locator` require at least one populated target signal.
- `WebStartBrowserParams`: Pydantic model for the explicit `start_browser` Web lifecycle capability. It accepts no fields.
- `WebCloseBrowserParams`: Pydantic model for the explicit `close_browser` Web lifecycle capability. It accepts no fields.
- `WebNavigateToParams`: Pydantic boundary model. Required url and optional waitUntil lifecycle state.
- `WebNavigateBackParams`: Pydantic boundary model. Optional waitUntil lifecycle state for back navigation.
- `WebClickOnParams`: Pydantic boundary model. Snapshot target or nonempty locator, with optional button and double fields.
- `WebTypeTextParams`: Pydantic boundary model. Required text, optional textType defaulting to literal, target or locator, and optional clear. Runtime-secret names resolve through Core before invocation.
- `WebSelectOptionParams`: Pydantic boundary model. Target or locator and at least one of value, label, index or values.
- `WebHoverOnParams`: Pydantic boundary model. Snapshot target or nonempty locator.
...
- `WebTakeScreenshotParams`: Pydantic boundary model with optional `fullPage` and `omitBackground` fields. Artifact paths remain storage-owned.
...
- `MacOSLaunchAppParams`: Pydantic model for `launch_app` driver parameters, including optional `bundle_id`, `app_path`, and `arguments` plus boolean `new_session` defaulting to false. Its field descriptions identify application identity inputs, session-creation-only inputs, and explicit session replacement for LLM callers. Runtime identity defaults come from Workspace target settings.
...
- `OpenAIAgentsSettings`: Pydantic runtime values for explicit user-configured Provider/model selection, tracing policy, turn limits, prompt templates and internal output policy. There is no default Provider or fixed model; Config supplies credentials and selected model from the user store. Provider metadata and credentials are not platform-YAML or environment fallbacks.
...
- `RuntimeSecretSettings`: Pydantic model listing Workspace runtime-secret reference names and private values. Values come from Workspace configuration, are validated before protected actions and resolved only in memory; serialized runtime settings and Cases do not contain the private values.
...
- `AndroidHarnessSettings`: Pydantic boundary model. Preset backend policy with Workspace application target and transient per-Run device serial. No environment target fallback.
- `WebHarnessSettings`: Pydantic boundary model. Preset Playwright policy with Workspace browser channel and discovered/configured executable path.
- `WindowsHarnessSettings`: Pydantic boundary model. Preset pywinauto/backend-kind policy and Workspace application path, window-title regex and launch arguments.
- `MacOSHarnessSettings`: Pydantic boundary model. Preset Mac2/snapshot/timeout policy with Workspace bundle/application identity. Only FSQ_MACOS_APPIUM_SERVER_URL overrides the preset endpoint.
...
## Run And Public Evidence Contracts

### Identity, outcome, and provenance

`RunMetadata` retains the identity, `source`, `result`, `runtime`, and `artifacts` fields used by existing Run queries. Version 2 also records the frozen execution-result reference, evidence health, processing summaries, provenance, and lineage references. `status` retains active values `preparing`, `running`, and `finalizing` and terminal values `success`, `failed`, `inconclusive`, `cancelled`, and `error`. Read-only history may project `interrupted` alongside the unchanged persisted status when owner loss is established. Liveness is separately `alive`, `dead`, or `unknown`; missing or stale ownership information alone does not establish a dead owner.

The frozen execution document identifies the mode-specific outcome and verification result independently. Strict evaluates required logical-step outcomes and evidence policy; Explore preserves earlier failed attempts while evaluating its final verification goal, fatal execution conditions, and evidence needed for that verdict. An SDK transport completion is never a successful action assertion. A zero-execution result without valid verification is inconclusive, not a passing test. A Case without an assertion may report completed execution but cannot claim that an unrequested business outcome was verified.

`RunResultSummary` exposes the bounded summary, logical-step counts, and first blocking failure identity consistently with the frozen execution result. Counts distinguish `passed`, `failed`, `skipped`, `cancelled`, and `incomplete`; their sum equals the number of logical leaf invocations accounted for. Executed attempt count is separate. Hook containers, synthetic progress, pre-plan items, and SDK summaries do not increment action counts. A dynamic failed attempt can remain inspectable even when the verified Goal succeeds.

Execution, verification, evidence, and processing use independent status domains. Evidence health is `complete`, `partial`, `unavailable`, or `not_applicable`, computed from required capture outcomes rather than screenshot count alone. Recording, report, suggestion, and publication processing each carry `not_requested`, `pending`, `success`, `failed`, or `cancelled`, safe diagnostics, and artifact references. A processing failure does not rewrite execution. Published reports retain both the original outcome and independent processing failure.

Provenance holds platform/backend and FSQ/runtime versions, available tested-application or repository revision, sanitized source snapshots with digests, and an allowlisted effective-settings summary. Unknown revision or version is explicit. Private values, full environment/configuration dumps, credential-bearing URLs, and digests of private values are absent. Digests identify the exact retained sanitized bytes, with transformation status; they do not falsely claim equality with an omitted original. Case/Goal source identities retain their bounded display fields and add contained snapshot references. Lineage relates originating Run, candidate/saved Case digest, replay Run, and the recorded command-to-execution mapping. Missing or mismatched source evidence produces an unresolved relation, not an inferred link.

### Steps, timing, and artifacts

`source_step_id` identifies a command within a Strict source snapshot. For Explore it identifies a safe capability-invocation proposal persisted before action, not a pre-plan index or a Case command invented after execution. Recording subsequently maps Case commands back to those identities. A logical invocation also has an occurrence-aware invocation path so repeated nested Cases remain separate; grouping dynamic attempts requires explicit invocation/attempt linkage rather than similarity of actions. Every actual attempt has a Run-unique `step_execution_id`; the compatibility `step_id` of a newly recorded result/event refers to that same execution identity. Planned Strict invocations can have no execution ID until attempted and carry an explicit skip or interruption reason. Historical IDs remain readable with ambiguity warnings rather than fabricated uniqueness. Retry limits do not prove that retries occurred.

Step results retain authored and canonical action names, step kind, source location, lifecycle phase, invocation occurrence, attempt index, action outcome, primary failure, independent evidence errors, and evidence references. A capture error cannot overwrite a known action failure. Attempt records are distinct from their logical-step aggregate; aggregate counts do not count retries twice. A skipped step records the blocking step or other reason, while interrupted work is not silently converted into skipped or passed.

New timing records use UTC start/end timestamps and monotonic measured durations. Step timing distinguishes prepare, invoke, configured settle delay, and finalize; capture timings are attributed within their containing phase and are not added twice. Measurements declare units, scope, and availability. A measured zero is valid; missing/uninstrumented/history-default values use null with an unavailable reason. Dynamic plan reconstruction timing is not actual per-action timing. AI usage records retain provider/model, request stage, actual reported quantities, and completeness; no estimated token totals or per-step allocation is synthesized.

Each artifact has a unique identity including kind and capture occurrence, contained relative path, MIME type, step execution/phase/capture reason, byte size and digest when available, and availability, transformation, and truncation metadata. Screenshot and UI snapshot captures remain separate outcomes so failure of one does not erase the other. A snapshot records known compaction/clipping limits; unknown historical coverage stays unknown. New `EvidenceBundle` manifests use `fsq.evidence/v2`, retain normalized steps/events/artifacts, and identify the journal sequence covered by the checkpoint. Readers accept supported legacy `1.0` manifests without rewriting them.

### Public report shape and export

`PublicRunReport` uses `schema_version`, `run`, `source`, `execution`, `verification`, `evidence`, `processing`, `steps`, `tool_calls`, `logs`, `artifacts`, `metrics`, `lineage`, `comparison`, and `warnings` sections. Each displayed failure or interpretation links to supporting execution or artifact identities and declares whether it is a recorded fact, deterministic classification, or persisted AI suggestion. Optional fields and sections carry availability; a missing report section is not equivalent to zero activity or successful verification.

`run.gate` is the derived report/CI disposition `passed`, `failed`, `error`, or `incomplete`, with ordered reason codes and supporting references; it never replaces `execution.outcome` or `verification.status`. `passed` requires a trustworthy completed success and satisfaction of the recorded required-evidence policy. Known blocking product/assertion failures produce `failed`; execution infrastructure failure, cancellation or confirmed interruption of unfinished execution/required verification, or missing required evidence produce `error`; active execution, inconclusive verification, or unresolved completion produce `incomplete`. If several conditions apply, `error` precedes `failed`, then `incomplete`, while the primary action failure remains separately visible. Unrequested verification and evidence are not retroactively required. Interruption solely during post-freeze processing/finalization retains the trustworthy frozen execution disposition and is shown separately as lifecycle/processing interruption; an unpersisted or untrustworthy freeze cannot use that exception. Recording, suggestion, publication, or rich-report processing failure alone does not change this disposition when the authoritative execution facts remain trustworthy. JUnit maps both `error` and `incomplete` to an error result.

Report source and artifact references are relative or identity-based and never expose absolute host paths. Log normalization preserves safe timestamp, sequence, event kind, tool, label, phase, execution identity, message, and actual result where available. Sensitive output stays redacted. Large text/artifact views expose truncation and a complete-artifact reference only when that artifact exists and is authorized.

Comparison distinguishes `before_after` from explicit `baseline_current`. Cross-Run matching uses validated source-step and occurrence identity, or persisted recording mappings; related reports require same-platform identity and digest validation. Unmatched steps, unavailable inputs, and clipped snapshots remain visible. Diff records describe observed changes and algorithm/normalization provenance, not an automatic assertion verdict. Aggregated reports preserve every independent Run outcome.

Cross-Run references are qualified by `run_id` plus `artifact_id` or `step_execution_id`; Workspace and platform come from the validated report scope. Local IDs alone never identify a baseline/related artifact. Bundle entries use `runs/<run_id>/` namespaces, and share selections use the same qualified references.

Export options are typed data, not a command or arbitrary executable template. Share transformations select allowed artifact identities, remove specified text/fields, or mask validated screenshot rectangles and produce a separate inventory with derived digests and transformation records. Secret replacement values are not embedded in the exported transformation log. No model validates a path by performing IO; Application and Report own destination/containment checks and file operations.

Share text/field transformations are limited to allowlisted display text and optional content; omission remains explicitly marked. Schema versions, identities, outcomes, gate, counts, measurement values, reference structure, recorded digests, and lineage semantics cannot be changed by a share profile. Derived-content digests are added separately. A profile may include a bounded `case_review_declaration` naming a qualified Case artifact and retained digest; it is labelled as a user declaration made for this export, never independent approval or a persisted Run fact, and cannot change the gate. Saving a Case alone is not review evidence.
...
- Web action targets are replayable semantic locators or current snapshot references. Parameter models reject unknown fields, including unsupported `element` descriptions. Screenshots are evidence/debugging observations rather than the normal action-selection substrate, and unsafe/opt-in capability families are not exposed.
...
- Windows launch defaults such as app path, pywinauto backend kind, window title regex, and configured launch arguments are runtime settings composed from Workspace target and preset policy by Config. `WindowsLaunchAppParams.extra_args` remains a per-step append-only override rather than a replacement for settings ownership.
...
- `_runs.py`: Shared Run metadata, frozen execution result, safe execution context, outcome, and provenance value models.
...
- `_report.py`: Report artifact, public report, and export boundary models.
...
- Runtime secrets are a model-owned allowlist of Workspace secret names with private in-memory values. Cases reference approved names through textType=runtimeSecret; events, artifacts, model-facing previews and reports exclude private values.
...
- SDK objects are not stored in shared models. Models retain serializable settings, tool/capability declarations and assertion evidence. Config composes the explicit user Provider/model and private credentials; runtime adapters construct SDK objects. No FSQ_LLM_PROVIDER or .env fallback participates in Provider selection.
