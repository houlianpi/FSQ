...
- `AndroidDevice`: Pydantic value model for one normalized `adb devices -l` record containing non-empty `serial` and `state` values plus safe string metadata.
- `AndroidDeviceDiscoveryResult`: Pydantic result model containing all normalized Android device records and an optional safe discovery error code/message. Successful discovery may contain zero devices; expected missing-ADB, timeout, process-start, and nonzero-exit failures are represented without entry-layer response or CLI wording.
...
- `AndroidInputTextParams`: Pydantic model for `input_text` parameters. It requires string `text`, optional serialized `textType` defaulting to `literal`, and either a `target` or non-empty `locator`. When `textType="runtimeSecret"`, `text` is an allowed workspace secret name resolved by `core` immediately before driver invocation; concrete Android drivers receive only resolved literal strings.
...
- `WebLocator`: Pydantic model for Web target locators with optional `role`, `name`, `text`, `label`, `placeholder`, `testId`, `css`, `xpath`, `altText`, `title`, and snapshot `ref` fields. Web action parameter models that accept either a semantic snapshot `target` or locator require at least one populated target signal. Compatible structured fields compose into one locator instead of silently discarding later fields.
...
- `WebClickOnParams`: Pydantic model for `click_on` parameters. It requires either a snapshot `target` or non-empty locator, with optional `double` and mouse `button` fields. Snapshot targets may contain role/name/ref annotations emitted by Web `ui_snapshot`; truncated display names are partial locator evidence rather than exact accessible names.
...
- `MacOSLaunchAppParams`: Pydantic model for `launch_app` driver parameters, including optional `bundle_id`, `app_path`, and `arguments` plus boolean `new_session` defaulting to false. Its field descriptions identify application identity inputs, session-creation-only inputs, and explicit session replacement for LLM callers. Runtime identity defaults come from the active workspace target.
...
- `OpenAIAgentsSettings`: Pydantic model for OpenAI Agents SDK provider configuration, including an explicit unconfigured `provider=None` state or effective `github_copilot`/`azure_openai` selection, tracing policy, turn limits, file-based prompt template customization, internal context trimming policy, internal AgentTool output artifact policy, and resolved private provider runtime values. Both configured providers use a non-empty user-selected model. Provider endpoint/key/token values are resolved by `config` from the user-level Provider store rather than platform YAML or environment values. The agent runtime uses the Responses API for configured model providers.
...
- `RuntimeSecretSettings`: Pydantic model containing the runtime-secret names available to text-entry commands and their private non-serialized values. The selected workspace platform config's `env` keys define the allowlist and values; concrete values remain in memory and never enter YAML cases, model-facing settings serialization, events, reports, or recordings.
...
- `AndroidHarnessSettings`: Pydantic model for Android runtime construction. The preset selects backend policy, the workspace target supplies `app_id`, and a private serial may carry an entry-layer device selection on a run-specific settings copy. Serial is not a YAML, environment, workspace, or case-metadata field. Strict-core execution does not enable AI assertion evaluators through this settings model.
- `WebHarnessSettings`: Pydantic model for Web runtime construction. The preset owns Playwright backend/headless/base-URL/viewport policy and the workspace target supplies one Playwright-documented Chromium distribution channel plus its required executable path. Strict-core execution does not enable AI assertion evaluators through this settings model.
- `WindowsHarnessSettings`: Pydantic model for Windows runtime construction. The preset selects `pywinauto` and the validated `uia | win32` adapter mode; the workspace target supplies application path, optional window title regex, and launch arguments. Strict-core execution does not enable AI assertion evaluators through this settings model.
- `MacOSHarnessSettings`: Pydantic model for macOS runtime construction. The preset supplies the YAML-backed `appium_mac2` backend, Appium server URL, and stable timeouts/snapshot policy; the workspace target supplies bundle id and/or app path. The shared aggregate may represent an unset server URL for non-macOS settings, while macOS platform validation requires a non-empty URL. Strict-core execution does not enable AI assertion evaluators through this settings model.
...
- `KnowledgeSkillSettings`: Pydantic model containing the repository-preset skill resource directory and configured `SkillConfig` items. Its nesting under agent-context settings is organizational and does not relocate skills beneath the workspace knowledge root.
...
- `AndroidWorkspaceTarget`, `WebWorkspaceTarget`, `WindowsWorkspaceTarget`, and `MacOSWorkspaceTarget`: Strict Pydantic target variants selected by workspace platform. They own application target values only and reject unknown fields. `WebWorkspaceTarget` stores any Playwright-documented Chromium distribution channel and its executable path; legacy version-2 Web targets that omit the channel normalize to `chrome`.
- `WorkspacePlatformCreateInput`: Strict extra-forbid Pydantic boundary model for one new workspace platform's platform id, matching target variant, and private runtime-secret `env`. It contains no workspace name or root path because Config owns new-root selection and persisted identity construction.
- `WorkspaceConfig`: Strict extra-forbid version-2 Pydantic boundary model for one platform's immutable workspace name/root/platform identity, matching target variant, and private runtime-secret `env`. The normalized absolute root is independent from the workspace-name basename. Names are trimmed bounded single-directory names and reject empty, dot segments, separators, control characters, host-invalid forms, and applicable Windows reserved device names. Env keys are valid environment-variable names and values are non-empty strings; duplicate YAML keys are rejected by config before model validation.
- `WorkspaceRegistryEntry`: Pydantic model containing one immutable registered workspace name and normalized absolute workspace root without platform, target, config-file, or secret data.
- `WorkspacePlatformStatus`: Pydantic safe-discovery model for one exact supported platform config, containing platform id, config path, `available` or `unavailable` status, safe message/action fields, and no target or env values.
...
- `WorkspaceInitResult`: Strict Pydantic safe-result model for CLI `init` orchestration. It contains status (`initialized`, `platform_added`, `unchanged`, or `updated`), canonical workspace name/root, and platform id. It contains no target fields, env names, env values, revisions, or Provider data.
...
- Web action parameter design follows Playwright MCP's LLM-facing core automation conventions where appropriate: action targets are replayable semantic snapshot references, composed locators, or stable unique selectors; screenshots are evidence/debugging observations rather than the normal action-selection substrate; and unsafe/opt-in capability families are not exposed.
...
- Windows launch defaults combine workspace-owned app path/window title/launch arguments with the preset-owned pywinauto backend kind. `WindowsLaunchAppParams.extra_args` remains a per-step append-only override rather than a replacement for settings ownership.
...
- `_devices.py`: Shared normalized Android device and discovery-result contracts.
...
- `_settings.py`: Settings value models, versioned workspace config/target models, registry and safe workspace result models, and private runtime-secret value contract.
...
- Android device discovery records are shared runtime facts, not entry-layer transport models. `models` owns their normalized shape; `core` owns discovery IO and parsing; CLI, Playground, and Control Plane own their selection and presentation policies.
...
- Config-level case lifecycle hook settings reuse the same `FsqCaseHook` entry models as case metadata. This keeps `caseLifecycle` configuration and `.fsq.yaml` lifecycle fields aligned while preserving module boundaries: `models` validates shape, `config` loads settings, and `cli` executes strict lifecycle behavior.
...
- Runtime-secret text references are represented by text-entry parameter fields, not by a separate pre-resolution `RuntimeSecretRef` object. Omitted `textType` means literal text for YAML compatibility; `textType="runtimeSecret"` means `text` is a workspace `env` key resolved by `core` before driver invocation.
...
- Runtime secrets are model-owned as a public-safe allowlist plus a private non-serialized value mapping derived from workspace `env`. This keeps credential values out of cases and serialized settings while allowing text-entry parameter models to reference approved names through `textType="runtimeSecret"`. Secret values are resolved only in memory by execution core and must be redacted from user-visible events, artifact output, model-facing previews, strict evidence, recording manifests, and final reports.
...
- A platform workspace config is a strict versioned boundary. `platform` and `target` must select matching discriminated target models; `root_path` must resolve to the containing workspace root; names and `env` keys/values obey config-owned validation; and shared models never perform filesystem IO. Config owns filename/platform agreement and consistency across sibling platform files.
- Workspace initialization results are public-safe cross-module facts. They identify the canonical workspace and selected platform plus the completed operation status without carrying target configuration, private env data, revisions, filesystem mutation details, or CLI presentation fields.
...
- OpenAI Agents SDK runtime objects are not stored directly in shared models. Models hold serializable configuration, AgentTool definitions, CommonTool/PlatformTool capability metadata, AI assertion request/results, and platform function schemas that `agent`, `tools`, `core`, and `providers` adapt into runtime objects. `OpenAIAgentsSettings.provider` stores `None` or the effective provider type after `config` applies the latest user snapshot. Provider endpoint/key/model values remain private runtime settings resolved from the user store; config-owned file records and provider-owned device-flow state do not become cross-module models unless another confirmed contract requires them.
- Skills are descriptive instruction bundles from repository-owned platform preset resources. `agent_context.knowledge.skills.dir` and `.items` describe those reusable preset bundles independently from the workspace `agent_context.knowledge.root_dir`. Skills do not grant CLI or shell execution. Model-facing skill context contains only successfully loaded skill instructions and not loader warnings.
- Agent context configuration groups project/page knowledge metadata and reusable skill definitions under `agent_context.knowledge`, but path composition keeps ownership separate: project knowledge and optional pre-plan page knowledge resolve under workspace `knowledge/<platform>/`, while preset skill resources remain repository-relative. Top-level `skills`, top-level `knowledge_dir`, and top-level `pre_plan` are not part of the public YAML surface.
- Parsed FSQ `.fsq.yaml` models are used for strict-core execution, including lifecycle hook metadata. `FsqCase.id` removes the complete `.fsq.yaml` terminal suffix from the already validated path. The public CLI's default LLM `--case-yaml` and `--case-dir` paths read YAML files as raw text and build goal/reference `Task` values without parsed FSQ models or hook execution.
