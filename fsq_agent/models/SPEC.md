...
- Windows action parameter design follows desktop conventions shared with macOS where possible: public aliases use `clickOn`, `doubleClickOn`, `rightClickOn`, `typeText`, `pressKey`, and `uiSnapshot`; coordinate-only actions are not exposed in the first Windows batch.
...
- Windows `ui_snapshot` is a driver-owned, read-only observation capability with canonical alias `uiSnapshot`. It returns a bounded pywinauto control-tree snapshot and must not reuse Android-oriented `ui_tree` or Web-oriented `page_snapshot` naming.
